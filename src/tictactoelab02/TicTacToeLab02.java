...
public class TicTacToeLab02{
		...
		do {
			...
			else
			{
				...
				if (ComputerToWinPositions != null)
				{
					...
					boolean canItBeUsed = pickingNumberCompNonRandomCheck(numberForCompToWin, board);   // valid move (empty slot)
					
					do
					{
						pickingNumberCompNonRandomCheck(numberForCompToWin, board);
					}
					while (!canItBeUsed);
	...
	public static boolean pickingNumberCompNonRandomCheck(int number, int[][] board)
	{
		if (!hasANumberBeenUsed(board, number))
		{
			return true; // true = go ahead and use the no.
		}
		return false; // false = don't use the no.
	}
	...
	public static int[] computerToBlock(boolean humanBooleanArray[], int board[][])
	{
		{
			// Below: because there is 5 options for the computer.
			// (Obviously one value in this array will already be true (used) after the first number
			// is placed on the board on the initial turn.
			for (int i = 0; i < 5; i++)
			{
				// Below: check: if the position in the array (from 0 to 4). If the particular value in
...

			{
				...
				{
					// Below: check: if the position in the array (from 0 to 4), if the value in the boolean array
					// is false (unused) then continue within:
